            ...
            <?php if (!empty($products)): ?>
                <?php foreach ($products as $product): ?>
                    ...
                    <div data-aos="fade-right" data-aos-easing="ease-in-sine" data-aos-duration="1500" data-aos-delay="250">
                        <a href="produit.html?p_id=<?= urlencode($product['p_id']) ?>" class="<?= $zoomClass ?>">
                            <img src="<?= htmlspecialchars($product['p_product_image_url']) ?>" alt="<?= htmlspecialchars($product['p_name']) ?>" class="<?= $imgClass ?>" />
                        </a>
                        <a href="produit.html?p_id=<?= urlencode($product['p_id']) ?>" class="text-a">
        ...
        <div class="container-marque">
            <?php if (!empty($brands)): ?>
                <?php foreach ($brands as $brand): ?>
                    <a href="marques.html?b_id=<?= urlencode($brand['b_id']) ?>" class="text-a zoom" data-aos="fade-left" data-aos-easing="ease-in-sine" data-aos-duration="1500">
